...
function new(class, ...)
	...
	callDerivedConstructor = function(self, instance, ...)
		for k, v in pairs(super(self)) do
			if v["virtual_constructor"] then
				v["virtual_constructor"](instance, ...)
			end
			local s = super(v)
...
function delete(self, ...)
	...
	callDerivedDestructor = function(parentClasses, instance, ...)
		for k, v in pairs(parentClasses) do
			if v["virtual_destructor"] then
				v["virtual_destructor"](instance, ...)
			end
			local s = super(v)
	...
	if(isElement(self)) then
		removeEventHandler(
			triggerClientEvent ~= nil and 
			"onElementDestroy" or
			"onClientElementDestroy", self, __removeElementIndex)
		destroyElement(self)
	end
...
--[[function addChangeHandler(instance, key, func)
	...
	if type(key) == "function" then
		...
		metatable.__changeData[key] = instance[key]
...
function __changeHandlerIndex(self, key)
	...
	return (
		...
		metatable.__realIndexFunction and
			metatable.__realIndexFunction(self["element"] or self, key) or
...
function __changeHandlerNewindex(self, key, value)
	...
	if type(metatable.__changeHandler) == "table" then
		if metatable.__changeHandler[key] then 
			local ret = metatable.__changeHandler[key](self["element"] or self, value)
			if ret ~= nil then
				value = ret
			end
	...
	elseif type(metatable.__changeHandler) == "function" then
		local ret = metatable.__changeHandler(self["element"] or self, key, value)
		if ret ~= nil then
			value = ret
		end
		metatable.__changeData[key] = value
	...
	return (
		...
		metatable.__realNewindexFunction and
			metatable.__realNewindexFunction(self["element"] or self, key, value) or
